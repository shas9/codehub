...
struct segtree
{
    ...
    void treeinit(ll node, ll b, ll e)
    {
        ...
        if(t1[left].first > t1[right].first) t1[node] = t1[left];
        else t1[node] = t1[right];
    }
    ...
    void treeinit2(ll node, ll b, ll e)
    {
        if(b == e)
        {
            t1[node] = {arr[b],b};
        ...
        treeinit2(left, b, mid);
        treeinit2(right, mid + 1, e);

        if(t1[left].first < t1[right].first) t1[node] = t1[left];
        else t1[node] = t1[right];

//        cout << b << " " << e << " is " << t1[node].first << " " << t1[node].second << "\n";
    }
