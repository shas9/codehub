...
{
	const ll inf = (1e16);
	const ll maxn = 305, maxe = 15005; //take node and edge twice in case node splitting
...
//    freopen("output.txt", "w", stdout);
    ...
    for(ll cs = 1; cs <= testcase; cs++)
    {
		...
		for(i = 1; i < (n + m); i++)
		{
			...
			if(i < min(n,m)) input -= i;
			else if(n + m - i < min(n,m)) input -= (n + m - i);
			else input -= min(n,m);

			dinic :: addedge(1, i + 1, input);
		}
		...
		for(i = 1; i < (n + m); i++)
		{
			...
			dinic :: addedge(i + (n + m), node, input);
		}
		...
		for(i = 1; i <= n; i++)
		{
			for(j = 1; j <= m; j++)
			{
				dinic :: addedge(i + j, typer[i][j], 99);
			}
		}
		...
		for(i = 1; i <= n; i++)
		{
			...
			{
				if(j > 1) printf(" ");
				printf("%lld", dinic :: flow[edgeno[i + j][typer[i][j]]] + 1);
