...
{
const ll inf = (1e16);
const ll maxn = 505, maxe = 35005; //take node and edge twice in case node splitting
...
//    freopen("output.txt", "w", stdout);
    ...
    for(ll cs = 1; cs <= testcase; cs++)
    {
        ...
        for(ll i = 1; i <= n; i++) slld(up[i]);
        for(ll i = 1; i <= n; i++) slld(down[i]);
        for(ll i = 1; i <= n; i++)
        {
            slld(input);

            if(input == -1) dinic :: addedge(i + 1, node,down[i]), dinic :: addedge(1, i + 1,dinic :: inf);
            else if(input == 1) dinic :: addedge(1, i + 1,up[i]), dinic :: addedge(i + 1, node,dinic :: inf);
            else
            {
                dinic :: addedge(i + 1, node,down[i]);
                dinic :: addedge(1, i + 1,up[i]);
            }
        }
