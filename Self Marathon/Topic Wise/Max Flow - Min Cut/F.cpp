...
{
	const ll inf = (1e16);
	const ll maxn = 305, maxe = 505; //take node and edge twice in case node splitting
...
//    freopen("output.txt", "w", stdout);
    ...
    for(ll cs = 1; cs <= testcase; cs++)
    {
		...
		for(i = 1; i <= 6; i++)
		{
			dinic :: addedge(m + 1 + i, m + 1 + i + 6, n);
			dinic :: addedge(m + 1 + i + 6, node, dinic :: inf);
		}
