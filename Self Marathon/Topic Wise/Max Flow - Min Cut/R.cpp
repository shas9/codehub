    ...
    for(ll cs = 1; cs <= testcase; cs++)
    {
		...
		for(i = 1; i <= n; i++)
		{
			for(j = 1; j <= m; j++)
			{
				...
				if(str[i][j - 1] == '*') parx[i][j] = parx[i][j - 1];
				else parx[i][j] = ++totx;
			}
		}
		...
		for(i = 1; i <= m; i++)
		{
			for(j = 1; j <= n; j++)
			{
				...
				if(str[j - 1][i] == '*') pary[j][i] = pary[j - 1][i];
				else pary[j][i] = ++toty;

			}
		}

		hkarp :: init(totx, toty);
		...
		for(i = 1; i <= n; i++)
		{
			for(j = 1; j <= m; j++)
			{
				...
				cout << i << " " << j << ": " << par[i][j] << " " << parx[i][j] << " " << pary[i][j] << endl;
//
//				hkarp :: add_edge(parx[i][j], par[i][j]);
//				hkarp :: add_edge(pary[i][j] + totx, par[i][j]);

				hkarp :: add_edge(parx[i][j], pary[i][j]);
			}
		}
