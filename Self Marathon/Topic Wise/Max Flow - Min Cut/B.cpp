...
{
	dinic :: init(0,snk,n * 2 + 1);

	for(ll i = 1; i <= n; i++)
	{
		dinic :: addedge(0, i, cc[i]);
		dinic :: addedge(i, i + n, dd[i]);
	}
	...
	for(ll i = 1; i <= n; i++)
	{
		for(ll j = 1; j <= n; j++)
		{
			...
			if(dist <= d || abs(d - dist) <= 0.000000000001) dinic :: addedge(i + n, j, dinic :: inf);
		}
	}

//	cout << snk << " " << dinic :: maxflow() << endl;

	return (dinic :: maxflow() == peng);
}
int main()
{
