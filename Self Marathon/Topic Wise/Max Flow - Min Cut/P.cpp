...
{
	const ll inf = (1e16);
	const ll maxn = 2200, maxe = 15005; //take node and edge twice in case node splitting
    ...
    while(cin >> n >> m)
    {
		...
		for(i = 1; i <= n; i++)
		{
			...
			for(j = 0; primes[j] * primes[j] <= input; j++)
			{
				...
				if(cnt)
				{
					g[i].push_back({primes[j], {++nodecnt, cnt}});

					dinic :: addedge(i + 1, nodecnt, cnt);
				}
			}

			if(input > 1)
			{
				g[i].push_back({input, {++nodecnt, 1}});

				dinic :: addedge(i + 1, nodecnt, 1);
			}

			dinic :: addedge(1, i + 1, dinic :: inf);
		}

//		for(i = 1; i <= n; i++)
//		{
//			for(auto it: g[i]) cout << i << ": " << it.first << " " << it.second.second << endl;
//		}
		...
		for(i = 1; i <= m; i++)
		{
			...
			for(auto it: g[u])
			{
				for(auto jt: g[v])
				{
					...
					{
						dinic :: addedge(it.second.first, ++nodecnt, min(it.second.second, jt.second.second));
						dinic :: addedge(jt.second.first, nodecnt, min(it.second.second, jt.second.second));
						dinic :: addedge(nodecnt, node,  2 * min(it.second.second, jt.second.second));
